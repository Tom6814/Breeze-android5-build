...
class AggregateSearchCubit extends Cubit<AggregateSearchState> {
  AggregateSearchCubit(
    ...
    Map<String, bool> initialSelectedSources = const <String, bool>{},
         ...
         AggregateSearchState(
           selectedSources: filterVisibleSourceSelections(
  ...
  Future<void> search() async {
    ...
    final selectedSources = filterVisibleSourceSelections(
      state.selectedSources,
    );
    if (selectedSources.length != state.selectedSources.length) {
      emit(state.copyWith(selectedSources: selectedSources));
    }
    final selected = selectedSources.entries
  ...
  Future<void> applySelectedSources(Map<String, bool> selectedSources) async {
    emit(
      state.copyWith(
        selectedSources: filterVisibleSourceSelections(
          Map<String, bool>.from(selectedSources),
        ),
      ),
    );
    await search();
